// Seeded simplex noise (2D + 3D) and helpers
...
export function hash3(x: number, y: number, z: number, seed: number) {
  let h = (x * 374761393 + y * 1103515245 + z * 668265263 + seed * 982451653) | 0;
  h = Math.imul(h ^ (h >>> 13), 1274126177);
  h = h ^ (h >>> 16);
  return (h >>> 0) / 4294967296;
}
...
export class Simplex {
  ...
  fbm2(x: number, y: number, oct: number, lac = 2, gain = 0.5): number {
    let a = 1, f = 1, s = 0, n = 0;
    for (let o = 0; o < oct; o++) {
